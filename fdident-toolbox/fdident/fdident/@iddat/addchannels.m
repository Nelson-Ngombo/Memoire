...
%ADDCHANNELS concatenate different channels
%       In ambiguity, the first object dominates (e.g. Name)
...
for ii=2:length(varargin)
  ...
  elseif ~isequal(get(Out,'groups'),get(dat2,'groups'))
    error('Groups must be the same in both objects')
  end
  if ii==2
    if strcmp(class(Out),'iddat'), Out=iddat(struct(Out)); end
  end
 
  %Check if inputs are allowed... (same or orthogonal)
  idata1=get(Out,'inputdata'); idata2=get(dat2,'inputdata');
  iname1=get(Out,'inputname'); iname2=get(dat2,'inputname');
  if isequal(idata1,idata2)&isequal(iname1,iname2)
    %same inputs
    if get(dat2,'inputchnumber')>0
      ch2=get(dat2,'ChTypes');
      ind=find(ch2=='o'+0);
      Struct.type='{}';
      Struct.subs={ind,':'};
      dat2=subsref(dat2,Struct); %add outputs only
    end
  elseif 0
    %orthogonal inputs, not ready
  end
  %Name, UserData stays that of dat1
  %Out.Notes='';
  addhist(Out,'Combination of two objects by channels')
  %
  ...
  Out.PeriodLength=pl;
  if isequal(fr1,fr2)
    if isnumeric(fr1), Out.Frequencies=fr1;
    elseif iscell(fr1), Out.Frequencies=[fr1;fr2];
    end
  else
