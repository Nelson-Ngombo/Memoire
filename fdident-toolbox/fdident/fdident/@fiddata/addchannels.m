...
%ADDCHANNELS  Unify data by adding channels
...
for iivn=2:length(varargin)
  ...
  if chn3<chn1+chn2
    if ~isequal(get(Out,'Inputfrequencies'),get(dat2,'Inputfrequencies'))
      error('Different inputfrequencies: not the same inputs')
    end
  end
  %
  ...
  if isequal(afp1,afp2)
    ...
    %AllFreqPoints is OK
    if isequal(fi1,fi2)&...
        ~iscell(fi1)|(iscell(fi1)&(size(fi1,1)==1)&(size(fi2,1)==1))
  ...
  for ivh=1:2
    ...
    if afpOK==1
      if isempty(cov1)&isempty(cov2) %do nothing
        ...
        if chn3<chn1+chn2 %input eliminated
          cht2=get(dat2,'chtype'); indi2=find(cht2=='i'+0);
          cov12(chno1+indi2,:,:)=[]; cov12(:,chno1+indi2,:)=[]; 
        end
...
%
% end of function @fiddata/addchannels
...
Out=1;
% end of file @fiddata/addchannels