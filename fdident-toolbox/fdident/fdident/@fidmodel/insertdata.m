...
if ~isa(data,'fidmodel')
  try
    ...
    error('Second input argument cannot be converted to fiddata')
  end
end
runmod=[]; devrunmod=[]; 
on=length(model.num)-1;
od=length(model.denom)-1;
dom=model.variable;
runmod.itmax=0;
runmod.initset='object';
runmod.initmodel=model;
runmod.plotdens=inf;
runmod.plot0='off';
devrunmod.displaymessages='off';
%
datamodel=elis(data,dom,on,od,runmod,devrunmod);
%
datamodel.fitinfo.iterations=NaN;
datamodel.fitinfo.termcause='';
datamodel.algorithm.iteration='';
% end ../@fidmodel/insertdata.m