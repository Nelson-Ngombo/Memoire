...
for ii=2:length(varargin)
  ...
  if isempty(ts1)&~isempty(ts2)
    Out.Ts=ts2;
    warning('Empty Ts in obj1, value taken from obj2 in @tiddata/merge') 
  elseif ~isempty(ts1)&isempty(ts2)
    warning('Empty Ts in obj2, value taken from obj1 in @tiddata/merge') 
  elseif ~isempty(ts2)
  ...
  if isempty(tst1)&~isempty(tst2)
    Out.TStart=tst2;
    warning('Empty TStart in obj1, values taken from obj2 in @tiddata/merge') 
  ...
  if isempty(st1)&~isempty(st2)
    Out.SampleTimes=st2;
    warning('Empty SamplingInstants in obj1, values taken from obj2 in @tiddata/merge') 
  elseif ~isempty(st2)&~isequal(st1,st2)
    error('Sampling times differ in objects')
  end 
  %
  Out.iddat=merge(Out.iddat,dat2.iddat);
end %for ii
%
