...
%HELP  Type out detailed help for tiddata objects
...
if nargin==1
  type @tiddata/tiddatah.m
else
  if ~isstr(prop)|(size(prop,1)~=1)
    error('prop is illegal')
  end
  prop=idpropch(tiddata,prop);
  ...
  if ~isempty(propi)
    ...
    [fid,errmess]=fopen('@tiddata/tiddatah.m','r');
  end
  ...
  if length(ind)~=1, ind=findstr(lower(filestr),[sprintf('\n'),'.',lower(prop),' ']); end
