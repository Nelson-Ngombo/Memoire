      ...
      elseif strcmp(command,'ui_help_this')
         ...
         switch WinTag
             case 'excitation_main'
                 doc Excitation_Signal_Design
             case 'gettime_main'
                 doc Read_Time_Domain_Data
             case 'getfreq_main'
                 doc Read_Frequency_Domain_Data
             case 'average_main'
                 doc Variances_and_Averaging
             case 'select_main'
                 doc Estimate_Plant_Model
             case 'aided_main'
                 doc Computer_Aided_Model_Scan
             case 'compare_main'
                 doc Evaluate_or_Compare_Plant_Models
         end
   ...
   if ~isempty(topic_index)
      ...
      switch Obj
          case "getting_started"
              helpwin getting_started
          case "fdtool_main"
              helpwin fdtool_main
          case "export-import"
              helpwin export_import
      end
      ...
      switch Obj
          ...
          otherwise
              ...
              Path = 'fdident/Help_Menu/';
              fname  = [Path,fname,'.m'];
              LongHelp = ['% ', LongHelp];
              LongHelp(end) = '';
              LongHelp_out = regexprep(LongHelp,'[\n\r]+','\n% ');
              fid = fopen(fname,'w');
              fprintf(fid, '%s', LongHelp_out);
              fclose(fid);
              helpwin(fname);
      end
   end
